use crate::{
    AppContext, BranchInputMode, CommitEntry, CommitFilesTarget, CommitInputMode, CommitListTarget,
    FileInputMode, SearchScope, branches, commit_file_tree_rows_for_read, commit_key,
    file_tree_rows_for_read, leave_commit_files_multi_select, leave_commit_multi_select,
    ...
    if scope == SearchScope::CommitFiles {
        match state.active_commit_files_target() {
            ...
            Some(CommitFilesTarget::Main)
                if state.ui.commits.files.mode == FileInputMode::MultiSelect =>
            {
                leave_commit_files_multi_select(
...
pub(crate) fn recompute_search_matches(state: &mut AppContext) {
    ...
    state.ui.search.matches = match scope {
        ...
        SearchScope::Commits => match state.active_commit_list_target() {
            Some(CommitListTarget::Branch) => collect_matches(
                state.repo.branches.commits.iter(),
                &query,
                commit_key,
                commit_search_text,
            ),
            Some(CommitListTarget::Main) | None => collect_matches(
        ...
        SearchScope::CommitFiles => {
            let rows = match state.active_commit_files_target() {
                Some(CommitFilesTarget::Branch) => commit_file_tree_rows_for_read(
                    &state.repo.branches.commit_files.items,
                    &state.ui.branches.commit_files,
                ),
                Some(CommitFilesTarget::Main) | None => commit_file_tree_rows_for_read(
                    &state.repo.commits.files.items,
...
fn select_current_search_match(state: &mut AppContext) -> bool {
    ...
    match scope {
        ...
        SearchScope::Commits => match state.active_commit_list_target() {
            Some(CommitListTarget::Branch) => select_index_by_key(
                &mut state.ui.branches.commits.selected,
                state.repo.branches.commits.iter().map(commit_key),
                &key,
            ),
            Some(CommitListTarget::Main) | None => select_index_by_key(
        ...
        SearchScope::CommitFiles => match state.active_commit_files_target() {
            Some(CommitFilesTarget::Branch) => select_commit_file_tree_path(
                &state.repo.branches.commit_files.items,
                &mut state.ui.branches.commit_files,
                &key,
            ),
            Some(CommitFilesTarget::Main) | None => select_commit_file_tree_path(
                &state.repo.commits.files.items,
                &mut state.ui.commits.files,
                &key,
            ),
